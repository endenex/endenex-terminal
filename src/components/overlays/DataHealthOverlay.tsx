  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-6" onClick={onClose}>
      <div className="bg-page border border-border rounded-lg shadow-2xl w-full max-w-5xl max-h-[88vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        <header className="flex-shrink-0 px-5 py-4 border-b border-border flex items-center bg-panel">
          <div>
            <p className="text-[11px] font-bold tracking-widest text-ink-3 uppercase">DATA HEALTH</p>
            <h2 className="text-[15px] font-semibold text-ink">Coverage, freshness &amp; source confidence</h2>
          </div>
            ...
            <>
              ...
              <div className="bg-panel border border-border rounded-lg overflow-hidden">
                <div className="px-4 py-3 border-b border-border">
                  <p className="text-[11px] font-semibold text-ink-2">Table coverage</p>
                </div>
                <table className="w-full text-[12px]">
                  <thead>
                    <tr className="border-b border-border bg-page">
                      <th className="px-4 py-2 text-left text-[10px] font-semibold text-ink-3 uppercase">Table</th>
                      <th className="px-4 py-2 text-right text-[10px] font-semibold text-ink-3 uppercase">Rows</th>
                      <th className="px-4 py-2 text-right text-[10px] font-semibold text-ink-3 uppercase">Latest</th>
                      <th className="px-4 py-2 text-right text-[10px] font-semibold text-ink-3 uppercase">Confidence (H/M/L)</th>
                      <th className="px-4 py-2 text-right text-[10px] font-semibold text-ink-3 uppercase">Derivation (Obs/Inf/Mod)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {tables.map((t, i) => {
                      ...
                      return (
                        <tr key={t.table} className={`border-b border-border last:border-0 ${i % 2 === 1 ? 'bg-page/50' : ''}`}>
                          <td className="px-4 py-2">
                            <div className="text-ink font-semibold">{t.label}</div>
                            <div className="text-[10px] text-ink-4">{t.table}</div>
                          </td>
                          <td className="px-4 py-2 text-right tabular-nums text-ink">{t.rows.toLocaleString('en-GB')}</td>
                          <td className={`px-4 py-2 text-right tabular-nums text-[11px] font-semibold ${
                            age.tone === 'fresh' ? 'text-up' :
                            age.tone === 'stale' ? 'text-highlight' :
                            'text-down'
                          }`}>{age.label}</td>
                          <td className="px-4 py-2 text-right tabular-nums text-[11px] text-ink-2">
              ...
              <div className="bg-panel border border-border rounded-lg overflow-hidden">
                <div className="px-4 py-3 border-b border-border">
                  <p className="text-[11px] font-semibold text-ink-2">Recent ingestion runs ({runs.length})</p>
                </div>
                {runs.length === 0 ? (
                  <p className="px-4 py-6 text-[11px] text-ink-3 text-center">No ingestion runs recorded yet. Runs are written by ingestion pipelines as they execute.</p>
                ) : (
                  <table className="w-full text-[12px]">
                    <thead>
                      <tr className="border-b border-border bg-page">
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-ink-3 uppercase">Pipeline</th>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-ink-3 uppercase">Status</th>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-ink-3 uppercase">Started</th>
                        <th className="px-4 py-2 text-right text-[10px] font-semibold text-ink-3 uppercase">Records</th>
                        <th className="px-4 py-2 text-left text-[10px] font-semibold text-ink-3 uppercase">Notes</th>
                      </tr>
                    </thead>
                    <tbody>
                      {runs.map((r, i) => (
                        <tr key={r.id} className={`border-b border-border last:border-0 ${i % 2 === 1 ? 'bg-page/50' : ''}`}>
                          <td className="px-4 py-2 text-ink font-semibold">{r.pipeline}</td>
                          ...
                          <td className="px-4 py-2 text-ink-2 text-[11px] tabular-nums">{new Date(r.started_at).toLocaleString('en-GB')}</td>
                          <td className="px-4 py-2 text-right tabular-nums text-ink-2">{r.records_written?.toLocaleString('en-GB') ?? '—'}</td>
                          <td className="px-4 py-2 text-ink-3 text-[10px]">{r.error_message ? <span className="text-down">{r.error_message}</span> : (r.notes ?? '—')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="bg-page border border-border rounded-lg p-4">
                <p className="text-[10px] font-semibold text-ink-3 uppercase tracking-wide mb-3">Source attributions</p>
                <div className="grid grid-cols-2 gap-3 text-[11px] text-ink-2">
